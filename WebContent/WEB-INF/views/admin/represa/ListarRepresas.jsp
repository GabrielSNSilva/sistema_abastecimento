		...
		<div class="row">
			<form  action="buscar_data" method="get">
				<div class="form-group col-md-5">
					<label>Listar por nome represa</label> <input 
						name="nome" class="form-control" id="nome" type="text" 
						required maxlength="100" value="${represa.nome }"
					...
					<button type="submit" class="btn btn-primary" name="command"
						value="buscar_data">Buscar</button>
				</div>
			</form>
		</div>
				...
				<table class="table table-striped">
					<thead>
						<tr>
							...
							<th>Sistema</th>
							<th>Volume Útil</th>
							<th>Volume Reserva Técnica</th>
							<th>Volume Armazenado</th>
							<th>Volume Total</th>
							<th>Calc indice1</th> 
							<th>Calc indice2</th> 
							<th>Calc indice3</th>
						...
						<c:forEach var="represa" items="${ represas }">
							<form action="visualizar_represa" method="get">
								<tr>
									<td>${ represa.data }</td>
									<td>${ represa.nome }</td>
									<td>${ represa.sistema }</td>
									<td>${ represa.volumeUtil }</td>
									<td>${ represa.volumeReservaTecnica }</td>
									<td>${ represa.volumeArmazenado }</td>
									<td>${ represa.volumeTotal }</td>
